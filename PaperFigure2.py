...
pacific_data = [np.load(path + f"FigureFiles/Pacific_{model}_ensemble_lfca_outputs.npz", allow_pickle=True) for model in models]
Forced_Unforced_TS_data = [xr.open_dataset(path + f'processed_runs/filtered_data/{model}_Pacific_forced_unforced_TS_25_modes.nc') for model in models]
...
ref_LFPs_plot[0, :2] = ref_LFPs_filled[0][:2]
...
ref_lfcs_plot[0, :, :2] = ref_lfcs[0][:,:2]
